...
""".format(n=self.name, pri=self.prizeMoney, c=category, phr=obscuredPhrase, gu=', '.join(sorted(guessed)))
        userinp = input(prompt)
        while True:
            if userinp == "pass" or userinp == "exit":
                break
            else:
                userinp = input(prompt)
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if len(possible) == 0:
            ...
            if coin == True:
                for word in WOFComputerPlayer.SORTED_FREQUENCIES:
                    if word in possible:
                        return word
            else:
